...
industry.data <- Translation.data %>% filter(EMPSTAT %in% c(10, 21))

industry.data <- industry.data %>% group_by(Date.year.month, indname) %>%
  mutate(count.EMPSTAT.10 = sum(EMPSTAT == 10),
         count.EMPSTAT.21 = sum(EMPSTAT == 21),
         )

industry.data <- industry.data %>% distinct(Date.year.month, covid.pre.post, .keep_all = TRUE)

# Graph data
Plot.point.2 <- ggplot(industry.data, aes(x = Date.year.month, y = count.EMPSTAT.10, color = indname)) +
  geom_point() +
  geom_vline(xintercept = ym('2020/03'), color = "orange", linetype = "dashed") +
  theme_minimal() +
