...
feols(Employment.Status.numeric ~ covid.pre.post*I(indname == 'Retail Trade')*Center.Date, weights = Translation.data$WTFINL)
```
...
Q2.reg1 <- Translation.data %>% 
  feols(Employment.Status.numeric ~ covid.pre.post*Retail.Trade.Employee*Center.Date, weights = Translation.data$WTFINL)
```
...
feols(Employment.Status.numeric ~ indname*Center.Date*covid.pre.post, weights = Translation.data$WTFINL)
```
...
feols(Employment.Status.numeric ~ Generation*Center.Date*covid.pre.post, weights = Translation.data$WTFINL)
```
...
Q3.Sex.reg <- Translation.data %>%
  feols(Employment.Status.numeric ~ Sex * Center.Date * covid.pre.post, weights = Translation.data$WTFINL)
```
...
Q3.Race.reg <- Translation.data %>%
  feols(Employment.Status.numeric ~ Race * Center.Date * covid.pre.post, weights = Translation.data$WTFINL)
```
...
Q3.Marital.reg <- Translation.data %>%
  feols(Employment.Status.numeric ~ Marital.Status * Center.Date * covid.pre.post, weights = Translation.data$WTFINL)
```
