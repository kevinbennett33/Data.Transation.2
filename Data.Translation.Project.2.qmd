...
```{r}
# Beta1 (covid.pre.post) is the RDD effect of covid.pre.post on the Retail industry.
```
...
```{r}
 # Aggregrate comparison: Beta 1 
Q2.reg1 <- Translation.data %>% 
...
```{r}
# Create df for number of people employed in each demographic, per month, to be used for visualization. 
...
```{r}
# NOTE: I want to add a first regression, as we did in Q1 an Q2. In other words, treat each demographic regression as having 2 parts: 1) How our reference group changed. 2) How our other groups within each demographic changed in comparison to our reference group.
```
...
feols(Employment.Status.numeric ~ Generation*Center.Date*covid.pre.post)
...
Q3.Sex.reg <- Translation.data %>%
  feols(Employment.Status.numeric ~ Sex * Center.Date * covid.pre.post)
...
Q3.Race.reg <- Translation.data %>%
  feols(Employment.Status.numeric ~ Race * Center.Date * covid.pre.post)
...
Q3.Marital.reg <- Translation.data %>%
  feols(Employment.Status.numeric ~ Marital.Status * Center.Date * covid.pre.post)
...
Format the writeup as a report with sections for the main questions, not a list of bullet points.
...
Your group presentation should be between 15 and 20 minutes long, plus a few minutes for questions. As many or as few of the people in your group can present as you like. There are no bonus points for being a presenter. 20 minutes is a *maximum* length.
